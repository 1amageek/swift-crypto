...
class DigestsTests: XCTestCase {
    func assertHashFunctionWithVector<H: HashFunction>(hf: H.Type, data: Data, testVector: String, file: StaticString = (#filePath), line: UInt = #line) throws {
        var h = hf.init()
        h.update(data: data)
    ...
    func assertHashFunctionCopyIsIndependent<H: HashFunction>(
        _ hashFunction: H.Type,
        file: StaticString = #filePath,
        line: UInt = #line
    ) {
        let commonPrefix: [UInt8] = [1, 2, 3, 4]
        var first = hashFunction.init()
        ...
        XCTAssertEqual(
            firstDigest,
            H.hash(data: commonPrefix + [5]),
            file: file,
            line: line
        )
        XCTAssertEqual(
            secondDigest,
            H.hash(data: commonPrefix + [6]),
            file: file,
            line: line
        )
        XCTAssertNotEqual(firstDigest, secondDigest, file: file, line: line)
    }

    func testHashFunctionCopiesDivergeIndependently() {
        assertHashFunctionCopyIsIndependent(Insecure.MD5.self)
        assertHashFunctionCopyIsIndependent(Insecure.SHA1.self)

...
Please open a pull request at https://github.com/1amageek/swift-crypto. Make sure the CI passes, and then wait for code review.

### Building the package

The package is validated with SwiftPM. The `Crypto` target depends on the sibling
`swift-ssl` checkout during development, and the release manifest must use the
tagged `swift-ssl` repository URL. Native, WASI, and Embedded WASI product builds
must all use the same Pure Swift implementation path.
...
### Pure Swift implementation boundary

New primitives belong in `swift-ssl/SSLCrypto`. The `Crypto` target should only add
a CryptoKit-shaped facade, typed errors, representation validation, and boundary
tests. Do not add C, Security.framework, or alternate backend selection paths.
